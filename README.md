...
Откатить последнюю миграцию - `yarn db:migrate:undo`.

Все ручки `/forum/*` закрыты авторизацией: без валидной сессии (кука, проверяемая через API Практикума) сервер отвечает 403. Автором топика/комментария становится текущий авторизованный пользователь.
