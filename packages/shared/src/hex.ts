...
export const hexToPixel = (coord: AxialCoord, size: number): Point => ({
  x: size * SQRT3 * (coord.q + coord.r / 2),
  y: size * 1.5 * coord.r,
})

export const pixelToHex = (point: Point, size: number): AxialCoord => {
...
export const hexNeighbors = (coord: AxialCoord): AxialCoord[] =>
  HEX_DIRECTIONS.map(direction => hexNeighbor(coord, direction))
...
export const isStraightLine = (a: AxialCoord, b: AxialCoord): boolean =>
  a.q === b.q || a.r === b.r || a.q + a.r === b.q + b.r

export const hexBetween = (a: AxialCoord, b: AxialCoord): AxialCoord => ({
  q: (a.q + b.q) / 2,
  r: (a.r + b.r) / 2,
})
...
export const hexagonCoords = (radius: number): AxialCoord[] => {
